...
## 基于 XML、Properties 配置加载Beans

加载入口方法:XMLBeanDefinitionReader#loadBeanDefinitions(String resourceUrl);
...
## 基于注解配置（推荐）
加载入口方法:ClassPathBeanDefinitionScanner#scan(String... basePackages),扫描路径加载有标记注解的类

配置了 @Compment、@Bean 等注解来实现,@Service 和 @Repository是@Compment 的衍生注解而已，功能一样。
...
BeanDefinitionReader,XML 解析器 - BeanDefnitionParser
